...
class _HttpRequester:
        ...
        if limit == "NA":
            ...
            while num_rows == limit:
                response = self._request_json(
                    path="/rest/v1/warehouse/raw-data?"
                    + url_params
                    + "&offset="
                    + str(offset)
                )
    ...
    def __assert_connection_to_xdmod_host(self):
        try:
            ...
            raise RuntimeError(
                "Could not connect to xdmod_host '" + self.__xdmod_host + "': " + str(e)
            ) from None
    ...
    def __request(self, path="", post_fields=None, stream=False):
        ...
        if response.status_code != 200:
            ...
            raise RuntimeError("Error " + str(response.status_code) + msg) from None
        if stream:
    ...
    def __get_data_post_fields(self, params):
        ...
        for dimension in params["filters"]:
            post_fields[dimension + "_filter"] = ",".join(params["filters"][dimension])
        return post_fields
